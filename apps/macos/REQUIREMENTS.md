...
| APP-7 | Verify signing, hardened runtime, notarization, release metadata, and install instructions before public distribution. | `make sign`; `make create-dmg`; `make release-readiness`; `make release-metadata-check` | Blocked until notarization credentials/profile and stable release metadata exist; `make release-readiness` verifies hardened runtime and aggregates the stable release metadata gate. |
...
Do not substitute a green test suite for signing, hardened runtime,
notarization, DMG install, release metadata, or real permission-flow proof.
`make release-readiness` must remain red until signed/notarized artifacts and
stable GitHub release metadata are all verified.

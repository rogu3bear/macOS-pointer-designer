...
| APP-7 | Verify signing, notarization, release metadata, and install instructions before public distribution. | `make sign`; `make create-dmg`; `make release-readiness`; `make release-metadata-check` | Blocked until notarization credentials/profile and stable release metadata exist; `make release-readiness` aggregates the stable release metadata gate. |
...
release metadata, or real permission-flow proof. `make release-readiness`
must remain red until signed/notarized artifacts and stable GitHub release
metadata are all verified.

...
echo "- APP-7 signing, notarization, Gatekeeper, release metadata, install instructions, optional Homebrew/cask truth, committed release source state, and artifact-bound human evidence: apps/macos/REQUIREMENTS.md -> make setup-notary-profile, make notary-profile-check, make release-source-state-check, make release-readiness, make release-metadata-check, manual-release-evidence-check.sh"
